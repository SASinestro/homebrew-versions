...
class Llvm39 < Formula
  ...
  def install
    ...
    (lib/"python2.7/site-packages").install "bindings/python/llvm" => "llvm-#{ver}", buildpath/"tools/clang/bindings/python/clang" => "clang-#{ver}"
    (lib/"python2.7/site-packages").install_symlink lib/"llvm-#{ver}/lib/python2.7/site-packages/lldb" => "lldb-#{ver}" if build.with? "lldb"

    Dir.glob(lib/"llvm-#{ver}/bin/*") do |exec_path|
      basename = File.basename(exec_path)
      bin.install_symlink exec_path => "#{basename}-#{ver}"
    end

    Dir.glob(lib/"llvm-#{ver}/share/man/man1/*") do |manpage|
